var cookieRemover = {
  removeCookies: function (mockRun) {
    feedback.updateHeader(
      mockRun ? 'Matching cookies (not removed):' : 'Cookies removed:'
    );
  ...
  removeCookiesFromDomain: function (domain, mockRun) {
    chrome.cookies.getAll({ domain: domain }, function (arrayOfCookies) {
      ...
      for (var i in arrayOfCookies) {
        ...
        var url =
          'http' + (cookie.secure ? 's' : '') + '://' + domain + cookie.path;
...
var feedback = {
  addUILine: function (msg) {
    $('#outputList').append($('<li>').append($('<span>').append(msg)));
  },
  appendToLastUILine: function (name) {
    $('#outputList li')
      .last()
      .append($('<span>').append('<br>&nbsp;&nbsp;' + name));
  },
...
$(document).ready(function () {
  ...
  $('#mockRun').on('change', function () {
    ...
    $('#removeCookies').val(
      isChecked ? 'Find matching cookies' : 'Remove matching cookies'
    );
  });
